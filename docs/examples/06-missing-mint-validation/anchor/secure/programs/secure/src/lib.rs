...
declare_id!("SECU6mintVa11d1d1d1d1d1d1d1d1d1d1d1d1d1d1");
...
pub struct Deposit<'info> {
    ...
    // FIX: Validate mint matches vault's expected mint
    #[account(
        mut,
    ...
    #[account(
        mut,
        constraint = user_token_account.mint == vault.token_mint @ VaultError::InvalidMint,
    )]
...
#[program]
pub mod secure_mint_validation {
    use super::*;
    ...
    pub fn deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
        // FIX: All accounts validated to use correct mint before transfer
        token::transfer(
            CpiContext::new(
    ...
    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        // FIX: Cannot withdraw vault tokens using wrong-mint balance
        token::transfer(
            CpiContext::new(
                ctx.accounts.token_program.to_account_info(),
                token::Transfer {
